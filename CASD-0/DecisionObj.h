...
This is a class whose instances wrap one decision. 
It has been implemented so we can use the Tube structure in the state of 
the world (State class). The Tube structure is an Array of objects, so we 
needed decisions to be objects.
...
@interface DecisionObj: SwarmObject {
  decision_t decision;
}

+create: (id <Zone>)z decision: (decision_t)dec;
